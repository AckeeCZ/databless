...
export type AttributeRelation2Type<P, S = never> = P extends { type: 'relation' } ? P extends { targetModel: 'self' } ? S extends Record<string, Attribute> ? Attributes2Entity<S> : P extends { targetModel: () => infer M } ? M extends Model ? Model2Entity<M> : never : never : never : never;
